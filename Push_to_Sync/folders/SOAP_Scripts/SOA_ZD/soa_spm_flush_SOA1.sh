...
#./check_all_status_Sync.sh

# Load the property file
#/opt/SP/oracle/ops-scripts/oracle/PauseResume/SPM_FLUSH/NEW_SCRIPT/SOA_ZD/soa_config.properties
/opt/oracle/teams/TA/folders/SOAP_Scripts/SOA_ZD/soa_config.properties

# Global varibale used for getProperty function
CONFIG_FILE_NAME=soa_config.properties
SERVER_HOST_PORT=8201

if [[ ($# == 0) || ($# > 1) ]]; then
  echo "Wrong number of arguments supplied. Need to provide one argument with any of one value(ALL,"$SOA_SERVER_LIST")"
  exit
fi

DATE=$(date +%Y%m%d%H%M%S)
LOG_FILE_NAME=./logs/SOA_SPM_FLUSH_LOG_$DATE
...
#          url="http://$SERVER_HOST_PORT$SPM_SCA_URL"
	  url="http://ENVHOST2:8201/soa-infra/services/default/AIASessionPoolManager/client"
...
if [[  $# == 1  ]]; then
     ...
     if [[  $1 == "ALL"  ]]; then
        ...
        echo "Invalid option supplied. Kindly provide one of the valid option (ALL,"$SOA_SERVER_LIST")"
        exit
    fi
fi
