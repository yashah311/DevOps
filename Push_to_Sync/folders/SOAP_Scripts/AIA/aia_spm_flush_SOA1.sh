...
CONFIG_FILE_NAME=aia_config.properties
SERVER_HOST_PORT=8101
...
#         url="http://$SERVER_HOST_PORT$SPM_SCA_URL"
	  url="http://ENVHOST1:8101/soa-infra/services/default/AIASessionPoolManager/client"
#         url="http://100.66.64.217:8102/soa-infra/services/default/AIASessionPoolManager/client"
          ...
          > response.xml
          curl -X POST -H "Content-Type: text/xml" -H "SOAPAction:$TERMINATE" -s -w "OPERATION:$SOAP_ACTION_TERMINATE|HTTPSTATUS:%{http_code}" --data @request_terminate.xml -o response.xml  -i $url
          echo -e '\n'
